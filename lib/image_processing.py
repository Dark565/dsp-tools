...
def move_image_subpixel(img: np.ndarray, move_vector: np.array, oversampling_res: np.array = None) -> np.ndarray:
  """
  ...
  if oversampling_res is not None:
    ...
    work_img = np.pad(img, ((pad_res[0], pad_res[0]), (pad_res[1], pad_res[1]), (0, 0)), mode='symmetric')
